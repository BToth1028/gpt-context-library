...
Files: code/stop-all-services.ps1

# Summary
Startup duplicates come from brittle window-title kills, surviving child processes, and restarts racing with teardown. Kill by **ports** and **process tree**, stop only the intended Docker stacks, and **verify ports are free** before proceeding. Fast and contained.

# What works
- Clear service list and target ports.
- Constraint to preserve Docker volumes is correct.

# What does not
- Title-based matching is unreliable.
- Children survive when only parents are killed.
- No verification loop before restart.

# Proposed solution
Use a single entrypoint function that:
1) Stops dockerized services first (Structurizr containers, Sourcegraph compose services).  
2) Kills known process families by command patterns.  
3) Kills owners of target ports using `taskkill /T /F` (process tree).  
4) Verifies target ports are free with bounded backoff (≤ 10 s).
...
# Implementation
Add `code/stop-all-services.ps1`. Call it at the **top** of your startup script:

```powershell
# start-all.ps1
& "$PSScriptRoot\stop-all-services.ps1"
if ($LASTEXITCODE -ne 0) { Write-Error "Pre-clean failed"; exit 1 }
```

# Verification
```powershell
# Should return $true and no target ports listed
. .\code\stop-all-services.ps1
Stop-AllProjectContextServices -Verbose
Get-NetTCPConnection | Where-Object { $_.LocalPort -in 8000,3000,7007,7080,8081 }
```

# Notes
- Preserves Docker volumes by using `docker compose stop` and `docker stop` for specific containers.
- Scope is limited to your ports and known process patterns to avoid collateral kills.
- Total cleanup typically < 2–4 s on a warm system; bounded at 10 s.
